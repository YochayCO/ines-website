Our main goal is to help social science researchers control the data of their surveys.
...
**The main data files we're working with:**
...
**Also, we're going to work with a DB.**

This DB was mostly created by the scripts in this project: `upsert_wxl_questions.py`, `upsert_question_instances.py` and `q_changes_extractor.py`. The only table not affected by these scripts is `Surveys` table, which was created mannualy. The other tables are `wxl_questions` and `q_instances` (the `qi_changes` table in the DB is no longer relevant - our output is in excels).
